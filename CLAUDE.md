...
## 指令模式（二選一）
- `/tdd <一或多個行為>` — 跑完 RED→GREEN→REFACTOR→COMMIT，中途不停，
  最後交出稽核報告（紅燈原文 + 測試檔 diff + 作弊自我稽核）。日常用這個。
  - 多個行為時會先產出「測試清單」並停下讓你確認排序，
    確認後每個行為各自跑一次完整循環、各自 commit。
  - 需求後面加 `--go` 可跳過清單確認直接開跑。
  - 一批上限 5 項。清單超過時會自動切批、只跑第 1 批，
    剩餘寫進 `docs/test-list.md` 供下次接續。
- `/red` → `/green` → `/refactor` — 三段各自停下等你確認。
  遇到設計不確定、需求模糊、或 `/tdd` 跑歪要重來時改用這個。
...
- `/red` 模式：停下等我確認才進 GREEN。`/tdd` 模式：直接續進 GREEN，不停
...
- 跑全部測試確認全綠，且未破壞既有測試
- `/red` 模式：停下等我確認。`/tdd` 模式：直接續進 REFACTOR，不停
...
## 通用
- 一個 commit = 一個行為。`/red` 模式綠燈後建議我 commit；`/tdd` 模式自動 commit
- 一次只做一個功能循環
